...
const DocumentsContent = ({ idParam }) => {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(
        `${process.env.base_url}/assets/documents/show/${documentId}`,
      ...
      setFormData({
        ...
        web_link: data.document.web_link || '',
        notes: data.document.notes || '',
        document: data.document.filepath,
        isPublic: data.document.isPublic,
      });
