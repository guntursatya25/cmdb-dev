...
const Index = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await axios.post(`${process.env.base_url}/masterdatas/categories`, {}, {
                headers: {
    ...
    const renderCell = (column, value, row) => {
        if (column === 'name') {
            ...
            return (
                <Link href={`/setup/categories/${row.id || '#'}`}>
                    {displayName}
                </Link>
            );
        }
    ...
    const headers = [ 'Name', 'Category', 'Description', 'Action'];
    const displayColumns = [ 'name', 'item_category', 'description', 'id'];
    ...
    const handleOpenForm = () => {
        router.push("/setup/categories/create");
    };
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const deleteRequests = selectedRows.map((rowId) => {
                    ...
                    return axios.delete(
                        `${process.env.base_url}/masterdatas/categories?id=${rowData.id}`,
                        {
                            headers: {
                                Authorization: `Bearer ${token}`,
