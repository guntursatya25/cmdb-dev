...
const Index = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await axios.post(
                `${process.env.base_url}/masterdatas/status-items`,
    ...
    const displayColumns = ['name', 'status', 'description', 'id'];
    ...
    const handleOpenForm = () => {
        router.push("/setup/status/create");
    };
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const deleteRequests = selectedRows.map((rowId) => {
                    ...
                    return axios.delete(
                        `${process.env.base_url}/masterdatas/status-items?id=${rowData.id}`,
                        {
                            headers: {
                                Authorization: `Bearer ${token}`,
