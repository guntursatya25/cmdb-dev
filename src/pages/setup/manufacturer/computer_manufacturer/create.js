...
const FormItemType = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append('Register ID', formData.registered_IDs);
    ...
    try {
      const response = await axios.post(
        `${process.env.base_url}/masterdatas/manufacturer/store`,
        data,
        {
          headers: {
        ...
        <div className="container">
          ...
          <div className="flex gap-[8%] w-full p-4">
            ...
            <div className="w-[50%] align-bottom content-end">
              <Input type='number' label="Register ID"  name="registered_IDs" value={formData.registered_IDs} onChange={handleInputChange} />
              <Input label="Type Register ID" name="type_registeredIDs" value={formData.type_registeredIDs} onChange={handleInputChange} />
            </div>
          </div>
          <hr />
