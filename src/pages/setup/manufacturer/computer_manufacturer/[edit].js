...
const Edit = () => {
    ...
    const breadcrumbs = [
        { link_prev: '/', prev: 'Home' },
        { link_prev: '/setup/manufacturer', prev: 'Manufacturer' },
    ];
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await axios.get(`${process.env.base_url}/masterdatas/manufacturers/show/${param}`,
                {
                    headers: {
            ...
            setFormData({
                ...
                type_registeredIDs: data.type_registeredIDs || '',
                registered_IDs: data.registered_IDs || '',
            });
        } catch (error) {
    ...
    const handleEdit = async (e) => {
        ...
        data.append('item_manufacturer', formData.item_manufacturer);
        data.append('registered_IDs', formData.registered_IDs);
        data.append('type_registeredIDs', formData.type_registeredIDs);

        const token = getCookie('access_token');

        try {
            const response = await axios.put(`${process.env.base_url}/masterdatas/manufacturers?id=${param}`, data, {
                headers: {
                    'Authorization': `Bearer ${token}`,
            ...
            setFormData({
                ...
                item_manufacturer: updatedData.item_manufacturer,
                type_registeredIDs: updatedData.type_registeredIDs,
                registered_IDs: updatedData.registered_IDs,
            });
    ...
    return (
        <Layout>
            ...
            <form onSubmit={handleEdit} className="work-page">
                {loading ? (
                    <div className="container flex justify-center w-full py-6">
                        <CircularProgress />
                    </div>
                ) : (
                    <div className="container">
                        <div className="flex justify-between p-4 w-full bg-gray-50 overflow-hidden rounded-t-md">
                            <h2 className="text-[#20288E] font-medium flex items-center">
                                ...
                                Item Manufacturer - {formData.name}
                            </h2>
                        </div>
                        <hr />
                        <div className="flex gap-[8%] w-full p-4">
                            <div className="w-[50%]">
                                ...
                                <Input label="Item Manufacturer" name="item_manufacturer" value={formData.item_manufacturer} onChange={handleInputChange} />
                            </div>
                            <div className="w-[50%] align-bottom content-end">
                                <Input label="Type Registered" name="type_registeredIDs" value={formData.type_registeredIDs} onChange={handleInputChange} />
                                <Input label="Register ID" name="registered_IDs" value={formData.registered_IDs} onChange={handleInputChange} />
                            </div>
                        </div>
                        <hr />
