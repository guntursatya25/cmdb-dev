...
const Index = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await axios.post(
                `${process.env.base_url}/incidents`,
            ...
            return (
                <p className="flex items-center justify-start">
                    <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 15 15">
                        <path fill={color} d="M9.875 7.5a2.375 2.375 0 1 1-4.75 0a2.375 2.375 0 0 1 4.75 0" />
                    </svg>
                    {value}
                </p>
            );
    ...
    const headers = ['Title', 'Priority', 'Time to Resolve', 'Assigned to', 'Last Update', 'Opening Date', 'Action'];
    const displayColumns = ['title', 'priority', 'resolvedDate', 'assigneeId', 'updated_at', 'created_at', 'id'];
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const deleteRequests = selectedRows.map((rowId) => {
                    ...
                    return axios.delete(
                        `${process.env.base_url}/incidents?id=${rowData.id}`,
                        {
                            headers: {
                                Authorization: `Bearer ${token}`,
