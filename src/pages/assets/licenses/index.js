...
const Index = () => {
    ...
    const fetchData = async (search = "") => {
        ...
        try {
            const response = await axios.post(`${process.env.base_url}/assets/licenses`, {
                page: currentPage,
                per_page: 10,
                search,
            }, {
    ...
    const renderCell = (column, value, row) => {
        if (column === 'name') {
            ...
            return (
                <Link href={`/assets/licenses/${row.id || '#'}`}>
                    {displayName}
                </Link>
            );
        }
    ...
    const headers = ['Name', 'Serial Number', 'Asset Number', 'Type'];
    const displayColumns = ['name', 'serial_number', 'number', 'md_type_license'];
